# Pre-release checklist

## Scientific freeze

- [x] Frozen 20-gene LPI definition versioned.
- [x] Phase 2B re-run from raw DepMap inputs.
- [x] Phase 3A PRISM/GDSC deterministic outputs audited.
- [x] iLINCS processed outputs independently re-summarized.
- [x] TCGA meta-analyses independently re-summarized.
- [x] Historical stale Phase 2B exports identified and excluded.
- [x] Fixed stochastic seed recorded (`20260915`).
- [x] No docking target nominated.

## Repository integrity

- [x] Raw-input SHA-256 manifest present.
- [x] Canonical output directories present.
- [x] Source scripts compile.
- [x] Notebooks are valid JSON.
- [x] Audit scripts return PASS.
- [x] Code license selected (MIT).
- [x] Release notes and release guide present.
- [x] Repository SHA-256 manifest generated for the frozen local archive.
- [x] GitHub Actions reproducibility audit passes on `main`.

## Author/admin items

- [x] Corresponding-author email recorded: `septiembre2103@hotmail.com`.
- [x] Author metadata populated in `CITATION.cff` (Juan José Pérez Cervera; ORCID 0000-0002-2657-6695).
- [x] Creator metadata populated in `.zenodo.json`.
- [x] Final public GitHub repository URL: `https://github.com/perez2103/laron-protection-index`.
- [x] Zenodo DOI published: `10.5281/zenodo.22771220`.
- [x] DOI inserted into repository metadata.
- [x] DOI-enriched v1.0.0 package published on Zenodo.
- [x] Manuscript Code availability updated to cite the public archive.
- [ ] Create GitHub release/tag `v1.0.0` pointing to the frozen manuscript-linked state.
- [x] Final author list/order confirmed: Juan José Pérez Cervera is the sole author; no institutional affiliation; no competing interests; no specific funding.
- [x] Juan José Pérez Cervera approved the manuscript on 2026-09-15 and accepts responsibility for the accuracy and integrity of the work.
